#!/usr/bin/env python3
"""
Manual script to resolve the merge conflict in all_deals.json
by creating a clean merged version.
"""
...
def extract_json_from_conflict(content):
    """Extract JSON parts from conflicted content."""
    lines = content.split('\n')
    ...
    for line in lines:
        ...
        if in_head:
            ...
            # Lines before first conflict marker
            if not head_lines and not local_lines:
                head_lines.append(line)
                local_lines.append(line)
...
def main():
    print("Manual merge conflict resolution for all_deals.json")
    print("=" * 60)
    ...
    # Extract JSON parts
    head_content, local_content = extract_json_from_conflict(content)
    
    print(f"HEAD content length: {len(head_content)} chars")
    print(f"Local content length: {len(local_content)} chars")
    
    # Parse JSON
    try:
        head_json = json.loads(head_content)
        print("✓ Successfully parsed HEAD JSON")
        ...
        head_content = re.sub(r',\s*]', ']', head_content)
        head_json = json.loads(head_content)
        print("✓ Fixed and parsed HEAD JSON")
    
    try:
        local_json = json.loads(local_content)
        print("✓ Successfully parsed local JSON")
    except json.JSONDecodeError as e:
        print(f"✗ Error parsing local JSON: {e}")
        # Try to fix by removing trailing commas
        local_content = re.sub(r',\s*}', '}', local_content)
        local_content = re.sub(r',\s*]', ']', local_content)
        local_json = json.loads(local_content)
        print("✓ Fixed and parsed local JSON")
    
    print(f"\nHEAD version has {len(head_json.get('deals', []))} deals")
    ...
    print(f"\nAdded {len(merged_deals)} manual deals from HEAD")
